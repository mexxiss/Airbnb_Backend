...
export const addStatement = async (req, res, next) => {
    ...
    const {url, title, total_amount, received_amount, net_amount_to_pay} = req.body;

    if (!url || !title || !total_amount || !received_amount || !net_amount_to_pay ) {
        return next(new apiError(400, "Incomplete or missing required fields"));
    }

    try {
        const newStatement = StatementModel.create({ url, title, total_amount, received_amount, net_amount_to_pay })
