...
export const SetGallaryType = async (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json(
        new apiResponse(200, gallaryType, "GallaryType Created Successfully")
      );
  } catch (error) {
...
export const GetAllGalleryTypes = async (req, res, next) => {
  // #swagger.tags = ['Admin']
  // #swagger.summary = "AUTHORIZED Admin can fetch all gallery types."

  try {
    const galleryTypes = await GallaryTypesModel.find({});
    return res
      .status(200)
      .json(new apiResponse(200, galleryTypes, "Fetched successfully"));
  } catch (error) {
    return next(new apiError(500, `Server Error: ${error}`));
  }
};
