...
export const getFigures = async (req, res, next) => {
  try {
    ...
    const activePropertiesCount = await PropertiesModel.countDocuments({
      status: "Active",
    });
    const inactivePropertiesCount = await PropertiesModel.countDocuments({
      status: "Inactive",
    });
    ...
    const totalBookings = cityBookings.reduce(
      (sum, city) => sum + city.bookingsCount,
      0
    );
