...
export const GetHomeContent = async (req, res, next) => {
    try {
        ...
        return res.status(200).json(new apiResponse(200, doc, "Home Content retrieved successfully"));
    } catch (error) {
...
export const SetHomeContent = async (req, res, next) => {
    ...
    try {
        const doc = await HomeContentModel.create({ banner_images, what_people_says, features });
        return res.status(200).json(new apiResponse(200, doc, "Home Content Added Successfully"));
    } catch (error) {
        return next(new apiError(500, "Server Error"));
    }
}
...
export const UpdateHomeContent = async (req, res, next) => {
    ...
    try {
        const doc = await HomeContentModel.updateOne({ _id: id }, { banner_images, what_people_says, features });
        return res.status(200).json(new apiResponse(200, doc, "Home Content updated successfully"));
    } catch (error) {
        return next(new apiError(500, "Server Error"));
    }
}